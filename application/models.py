...
class User(database.Model, UserMixin):
    ...
    def get_token(self, subject, expires_in=600):
        claims = {'sub': subject, 'id': self.user_id,
                  'exp': time() + expires_in}
    ...
    def verify_token(token, subject):
        try:
            ...
            user_id = claims.get('id') if claims.get(
                'sub') == subject else None
            if not user_id:
                return
...
list_category = database.Table(
    'list_category', database.metadata,
    database.Column('list_id', database.Integer,
...
list_item = database.Table(
    'list_item', database.metadata,
    database.Column('list_id', database.Integer,
                    database.ForeignKey('list.list_id'),
                    nullable=False, index=True),
    database.Column('item_id', database.Integer,
...
class Item(database.Model):
    ...
    # one to many: category <-> item
    category_id = database.Column(database.Integer, database.ForeignKey(
