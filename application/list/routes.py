...
def create():
    ...
    return render_template(
        'list/create.html', title='Create List', form=form,
...
def update(list_id):
    ...
    return render_template(
        'list/update.html', title='Update List', form=form,
        cancel=url_for('list.list'))
...
def delete(list_id):
    ...
    return render_template(
        'list/delete.html', title='Delete List', form=form,
        cancel=url_for('list.list'))
...
def category(list_id):
    ...
    categories = \
        ...
        .outerjoin(list_category,
                   and_(list_category.c.category_id == Category.category_id,
                        list_category.c.list_id == list_id)) \
        .order_by(Category.name).all()

    return render_template(
        'list/category/category.html', title='Categories in List',
        list=lst, categories=categories, cancel=url_for('list.list'))
...
def category_get():
    ...
    if Category.query.filter(Category.lists.any(list_id=list_id)) \
...
def warn_unchecked_categories(list_id):
    ...
    checked_categories = \
        database.session.query(Category.category_id) \
        .join(list_category,
              and_(list_category.c.category_id == Category.category_id,
                   list_category.c.list_id == list_id))
    ...
    unchecked_categories = \
        ...
        .join(Item) \
        .join(list_item,
              and_(list_item.c.item_id == Item.item_id,
                   list_item.c.list_id == list_id)) \
...
def item(list_id):
    ...
    return render_template(
        'list/item/item.html', title='Items in List', list=lst,
        items_categories=items_categories, cancel=url_for('list.list'))
...
def item_get():
    ...
    if Item.query.filter(Item.lists.any(list_id=list_id)) \
